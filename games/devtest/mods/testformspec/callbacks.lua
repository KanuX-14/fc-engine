...
local out = function(player, formname, fields, number)
	...
	freecraft.chat_send_player(player:get_player_name(), msg)
...
freecraft.register_chatcommand("test_formspec_callbacks", {
	params = "[ 0 | 1 | 2 ]",
	...
	func = function(name, param)
		...
		if callback_test == 1 then
			freecraft.chat_send_player(name, "Formspec callback test mode 1 enabled: Logging only")
		elseif callback_test == 2 then
			freecraft.chat_send_player(name, "Formspec callback test mode 2 enabled: Three callbacks, disable pre-registered callbacks")
		else
			callback_test = 0
			freecraft.chat_send_player(name, "Formspec callback test disabled!")
		end
	end
})
