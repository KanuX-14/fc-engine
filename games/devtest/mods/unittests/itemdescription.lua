...
freecraft.register_tool("unittests:description_test", {
	description = full_description,
	inventory_image = "unittests_description_test.png",
	groups = { dummy = 1 },
})

freecraft.register_chatcommand("item_description", {
	param = "",
...
local function test_short_desc()
	...
	assert(freecraft.registered_items["unittests:description_test"].short_description == nil)
	assert(stack:get_description() == full_description)
	assert(stack:get_description() == freecraft.registered_items["unittests:description_test"].description)
