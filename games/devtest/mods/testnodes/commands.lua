...
local function place_nodes(param)
	...
	table.sort(nodes)
	freecraft.chat_send_player(name, "Placing nodes …")
	local nodes_placed = 0
	local aborted = false
	for n=1, #nodes do
		local itemstring = nodes[n]
		local def = freecraft.registered_nodes[itemstring]
		local p2_max = 0
		...
		for p2 = 0, p2_max do
			...
			if not ((def.paramtype2 == "meshoptions" and p2 % 8 > 4) or
				...
				freecraft.set_node(pos, { name = itemstring, param2 = p2 })
	...
	if aborted then
		freecraft.chat_send_player(name, "Not all nodes could be placed, please move further away from the world boundary. Nodes placed: "..nodes_placed)
	end
	freecraft.chat_send_player(name, "Nodes placed: "..nodes_placed..".")
end
...
freecraft.register_chatcommand("test_place_nodes", {
	params = "[ no_param2 ]",
	...
	func = function(name, param)
		...
		for itemstring, def in pairs(freecraft.registered_nodes) do
			if itemstring ~= "ignore" and
					-- Skip callback test and dummy nodes
					-- to avoid clutter and chat spam
					freecraft.get_item_group(itemstring, "callback_test") == 0 and
					freecraft.get_item_group(itemstring, "dummy") == 0 then
		...
		freecraft.emerge_area(start_pos,
