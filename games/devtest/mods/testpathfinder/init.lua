local S = freecraft.get_translator("testpathfinder")
...
local function find_path_for_player(player, itemstack)
	...
	if x and y and z then
		...
		if (not IGNORE_MAX_DISTANCE_SAFEGUARD) and (vector.distance(pos1, pos2) > MAX_DIRECT_DISTANCE) then
			freecraft.chat_send_player(player:get_player_name(), S("Destination too far away! Set a destination (via placing) within a distance of @1 and try again!", MAX_DIRECT_DISTANCE))
			return
		end
		local str = S("Path from @1 to @2:",
			freecraft.pos_to_string(pos1),
			freecraft.pos_to_string(pos2))

		freecraft.chat_send_player(player:get_player_name(), str)
		local time_start = freecraft.get_us_time()
		local path = freecraft.find_path(pos1, pos2, MAX_SEARCH_DISTANCE, MAX_JUMP, MAX_DROP, algo)
		local time_end = freecraft.get_us_time()
		local time_diff = time_end - time_start
		str = ""
		if not path then
			freecraft.chat_send_player(player:get_player_name(), S("No path!"))
			freecraft.chat_send_player(player:get_player_name(), S("Time: @1 ms", time_diff/1000))
			return
		end
		for s=1, #path do
			str = str .. freecraft.pos_to_string(path[s]) .. "\n"
			local t
			if s == #path then
			...
			freecraft.add_particle({
				pos = path[s],
				expirationtime = 5 + 0.2 * s,
				playername = player:get_player_name(),
				glow = freecraft.LIGHT_MAX,
				texture = t,
				size = 3,
			})
		end
		freecraft.chat_send_player(player:get_player_name(), str)
		freecraft.chat_send_player(player:get_player_name(), S("Path length: @1", #path))
		freecraft.chat_send_player(player:get_player_name(), S("Time: @1 ms", time_diff/1000))
	end
end
...
local function set_destination(itemstack, user, pointed_thing)
	...
	if pointed_thing.type == "node" then
		...
		freecraft.chat_send_player(user:get_player_name(), S("Destination set to @1", freecraft.pos_to_string(pos)))
		return itemstack
	end
end
...
local function find_path_or_set_algorithm(itemstack, user, pointed_thing)
	...
	-- Sneak: Set algorithm
		...
		freecraft.chat_send_player(user:get_player_name(), S("Algorithm: @1", algorithms[algo+1]))
		return itemstack
	end
end
...
freecraft.register_tool("testpathfinder:testpathfinder", {
