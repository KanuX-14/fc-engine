...
local S = freecraft.get_translator("chest_of_everything")
...
local function add_detached_inventories(player)
	...
	local inv_items = freecraft.create_detached_inventory("chest_of_everything_items_"..name, {
	...
	local inv_trash = freecraft.create_detached_inventory("chest_of_everything_trash_"..name, {
		allow_take = function(inv, listname, index, stack, player)
			return 0
		end,
		allow_move = function(inv, from_list, from_index, to_list, to_index, count, player)
...
local sort_items_by_type = function(item1, item2)
	...
	* Dummy items ]]
	local def1 = freecraft.registered_items[item1]
	local def2 = freecraft.registered_items[item2]
	local tool1 = def1.type == "tool"
	local tool2 = def2.type == "tool"
	local testtool1 = freecraft.get_item_group(item1, "testtool") == 1
	local testtool2 = freecraft.get_item_group(item2, "testtool") == 1
	local dummy1 = freecraft.get_item_group(item1, "dummy") == 1
	local dummy2 = freecraft.get_item_group(item2, "dummy") == 1
...
local collect_items = function(filter, lang_code)
	...
	for itemstring, def in pairs(freecraft.registered_items) do
		if itemstring ~= "" and itemstring ~= "unknown" and itemstring ~= "ignore" then
			if filter and lang_code then
				...
				if desc ~= "" then
					...
					if not matches then
						local tdesc = freecraft.get_translated_string(lang_code, desc)
						if tdesc ~= "" then
					...
					if not matches then
						...
						if sdesc ~= "" then
							sdesc = freecraft.get_translated_string(lang_code, sdesc)
...
local function update_inventory(name)
	...
	if search == "" then
		...
		local lang_code = freecraft.get_player_information(name).lang_code
...
local show_formspec = function(name)
	...
	freecraft.show_formspec(name, "chest_of_everything:getitem", form)
	return true
end
...
freecraft.register_tool("chest_of_everything:bag", {
...
freecraft.register_node("chest_of_everything:chest", {
