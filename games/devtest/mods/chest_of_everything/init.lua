...
local inv_everything = freecraft.create_detached_inventory("everything", {
...
local inv_trash = freecraft.create_detached_inventory("trash", {
	allow_take = function(inv, listname, index, stack, player)
		return 0
	end,
	allow_move = function(inv, from_list, from_index, to_list, to_index, count, player)
...
freecraft.register_on_mods_loaded(function()
	local items = {}
	for itemstring,_ in pairs(freecraft.registered_items) do
