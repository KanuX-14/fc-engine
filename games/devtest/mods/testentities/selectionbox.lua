...
freecraft.register_entity("testentities:selectionbox", {
	initial_properties = {
		visual = "cube",
...
local hud_ids = {}
freecraft.register_globalstep(function()
	if active_selectionbox_entities == 0 then
		return
	end

	for _, player in pairs(freecraft.get_connected_players()) do
		local offset = player:get_eye_offset()
		offset.y = offset.y + player:get_properties().eye_height
		local pos1 = vector.add(player:get_pos(), offset)
		local raycast = freecraft.raycast(pos1, vector.add(pos1, vector.multiply(player:get_look_dir(), 10)), true, false)
