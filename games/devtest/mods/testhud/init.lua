...
freecraft.register_globalstep(function(dtime)
	...
	font_etime = 0
	for _, player in ipairs(freecraft.get_connected_players()) do
...
freecraft.register_chatcommand("hudfonts", {
	params = "[<HUD elements>]",
	...
	func = function(name, param)
		...
		if player_font_huds[name] == nil then
			...
			freecraft.chat_send_player(name, ("%d text HUD element(s) added."):format(param))
		else
			...
			if huds then
				...
				freecraft.chat_send_player(name, "All text HUD elements removed.")
			end
...
freecraft.register_chatcommand("hudwaypoints", {
	...
	func = function(name, params)
		...
		local function change(chplayer)
			...
			freecraft.chat_send_player(chplayer:get_player_name(), "Waypoints changed.")
		end
		if params == "add_change" then
			-- change immediate
			change(player)
		else
			freecraft.after(0.5, change, player)
		end
...
freecraft.register_chatcommand("zoomfov", {
	params = "[<FOV>]",
