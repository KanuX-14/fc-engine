FreeCraft Lua Modding API Reference
==================================

* More information at <http://www.minetest.net/>
* Developer Wiki: <http://dev.minetest.net/>
* (Unofficial) FreeCraft Modding Book by rubenwardy: <https://rubenwardy.com/minetest_modding_book/>

Introduction
------------

Content and functionality can be added to FreeCraft using Lua scripting
in run-time loaded mods.

A mod is a self-contained bunch of scripts, textures and other related
things, which is loaded by and interfaces with FreeCraft.
...
FreeCraft keeps and looks for files mostly in two paths. `path_share` or `path_user`.
...
`/usr/share/freecraft` while the user path resides in `.freecraft` in the home directory.
...
* `game.conf`, with the following keys:
    * `title`: Required, a human-readable title to address the game, e.g. `title = FreeCraft Game`.
...
* `name`: The modpack name. Allows FreeCraft to determine the modpack name even
...
`freecraft.get_modpath(modname)`.

### mod.conf

A `Settings` file that provides meta information about the mod.

* `name`: The mod name. Allows FreeCraft to determine the mod name even if the
...
wants to register. Subsequent execution depends on freecraft calling the
registered callbacks.

`freecraft.settings` can be used to read custom or existing settings at load
...
`freecraft.register_alias(alias, original_name)` or
`freecraft.register_alias_force(alias, original_name)`.
...
The only difference between `freecraft.register_alias` and
`freecraft.register_alias_force` is that if an item named `alias` already exists,
`freecraft.register_alias` will do nothing while
`freecraft.register_alias_force` will unregister it.
...
you have an item called `epiclylongmodname:stuff`, you could do
    ...
    freecraft.register_alias("stuff", "epiclylongmodname:stuff")
...
of the game's nodes are to be used for core mapgen generation. For example:
    ...
    freecraft.register_alias("mapgen_stone", "default:stone")
...
By default the world is filled with air nodes. To set a different node use e.g.:
    ...
    freecraft.register_alias("mapgen_singlenode", "default:stone")
...
`freecraft.encode_base64(freecraft.encode_png(tex))`,
...
In particular consider `freecraft.dynamic_add_media` and test whether
...
```lua
freecraft.register_node("mod:stone", {
    description = "Stone",
...
freecraft.register_craft({
    output = freecraft.itemstring_with_palette("wool:block", 3),
    type = "shapeless",
    recipe = {
        "wool:block",
        "dye:red",
...
To set the `color` field, you can use `freecraft.itemstring_with_color`.
...
whole tile. Soft texture overlays were added to FreeCraft to allow
...
```lua
freecraft.register_node("default:dirt_with_grass", {
...
All nodes registered with `freecraft.register_node` get added to the table
`freecraft.registered_nodes`.

If you want to check the drawtype of a node, you could do it like this:

```lua
local def = freecraft.registered_nodes[nodename]
...
```lua
freecraft.registered_nodes[node.name]
```
...
* `paramtype2 = "flowingliquid"`
    ...
    * Bits 0-2: Liquid level (0-7). The higher, the more liquid is in this node;
      see `freecraft.get_node_level`, `freecraft.set_node_level` and `freecraft.add_node_level`
...
* `paramtype2 = "wallmounted"`
    ...
    * You can make this value by using `freecraft.dir_to_wallmounted()`
    * Values range 0 - 5
...
* `paramtype2 = "facedir"`
    ...
    * Can be made by using `freecraft.dir_to_facedir()`.
...
* `paramtype2 = "4dir"`
    ...
    * Can be made by using `freecraft.dir_to_fourdir()`.
...
Look for examples in `games/devtest` or `games/freecraft_game`.
...
Additionally the methods `freecraft.itemstring_with_palette(item, palette_index)`
and `freecraft.itemstring_with_color(item, colorstring)` may be used to create
...
```lua
freecraft.get_item_group(itemname, groupname)
```
...
use `freecraft.formspec_escape`.
For colored text you can use `freecraft.colorize`.
...
**WARNING**: FreeCraft allows you to add elements to every single formspec instance
...
* See also `freecraft.explode_textlist_event`
...
* See also `freecraft.explode_scrollbar_event`
...
* See also `freecraft.explode_table_event`
...
* `freecraft.get_color_escape_sequence(color)`:
    * `color` is a ColorString
    * The escape sequence sets the text color to `color`
* `freecraft.colorize(color, message)`:
    * Equivalent to:
      `freecraft.get_color_escape_sequence(color) ..
      message ..
      freecraft.get_color_escape_sequence("#ffffff")`
* `freecraft.get_background_escape_sequence(color)`
    * `color` is a ColorString
    * The escape sequence sets the background of the whole text element to
      `color`. Only defined for item descriptions and tooltips.
* `freecraft.strip_foreground_colors(str)`
    * Removes foreground colors added by `get_color_escape_sequence`.
* `freecraft.strip_background_colors(str)`
    * Removes background colors added by `get_background_escape_sequence`.
* `freecraft.strip_colors(str)`
...
FreeCraft stores 3-dimensional spatial vectors in Lua as tables of 3 coordinates,
...
Since FreeCraft 5.5.0, vectors additionally have a metatable to enable easier use.
...
should be used to ensure seamless compatibility between mods and the FreeCraft API.
...
* `freecraft.hash_node_position` (Only works on node positions.)
* `freecraft.dir_to_wallmounted` (Involves wallmounted param2 values.)
...
* `freecraft.wrap_text(str, limit, as_table)`: returns a string or table
...
* `freecraft.pos_to_string(pos, decimal_places)`: returns string `"(X,Y,Z)"`
...
* `freecraft.string_to_pos(string)`: returns a position or `nil`
    * Same but in reverse.
    * If the string can't be parsed to a position, nothing is returned.
* `freecraft.string_to_area("(X1, Y1, Z1) (X2, Y2, Z2)", relative_to)`:
    ...
    * Example: `freecraft.string_to_area("(1,2,3) (~5,~-5,~)", {x=10,y=10,z=10})`
      returns `{x=1,y=2,z=3}, {x=15,y=5,z=10}`
* `freecraft.formspec_escape(string)`: returns a string
    * escapes the characters "[", "]", "\", "," and ";", which cannot be used
      in formspecs.
* `freecraft.is_yes(arg)`
    * returns true if passed 'y', 'yes', 'true' or a number that isn't zero.
* `freecraft.is_nan(arg)`
    * returns true when the passed number represents NaN.
* `freecraft.get_us_time()`
...
* `table.shuffle(table, [from], [to], [random_func])`:
    ...
    * `random_func` defaults to `math.random`. This function receives two
      ...
      between them.
* `freecraft.pointed_thing_to_face_pos(placer, pointed_thing)`: returns a
  position.
    * returns the exact position on the surface of a pointed node
* `freecraft.get_tool_wear_after_use(uses [, initial_wear])`
...
* `freecraft.get_dig_params(groups, tool_capabilities [, wear])`:
...
* `freecraft.get_hit_params(groups, tool_capabilities [, time_from_last_punch [, wear]])`:
...
Texts can be translated client-side with the help of `freecraft.translate` and
translation files.
...
Two functions are provided to translate strings: `freecraft.translate` and
`freecraft.get_translator`.

* `freecraft.get_translator(textdomain)` is a simple wrapper around
  `freecraft.translate`, and `freecraft.get_translator(textdomain)(str, ...)` is
  equivalent to `freecraft.translate(textdomain, str, ...)`.
  It is intended to be used in the following way, so that it avoids verbose
  repetitions of `freecraft.translate`:

  ```lua
  local S = freecraft.get_translator(textdomain)
  S(str, ...)
  ```

  As an extra commodity, if `textdomain` is nil, it is assumed to be "" instead.

* `freecraft.translate(textdomain, str, ...)` translates the string `str` with
...
The output of `freecraft.translate` is a string, with escape sequences adding
...
operations such as `freecraft.colorize` which are also concatenation.
...
* `@=` acts as a literal `=`. It is not required in strings given to
  `freecraft.translate`, but is in translation files to avoid being confused
  with the `=` separating the original from the translation.
* `@\n` (where the `\n` is a literal newline) acts as a literal newline.
  As with `@=`, this escape is not required in strings given to
  `freecraft.translate`, but is in translation files.
...
`freecraft.get_translated_string(lang_code, string)`: Translates `string` using
...
the table returned by `freecraft.get_player_information(name)`.
...
Usually in FreeCraft the input values are either 2D or 3D co-ordinates in nodes.
...
FreeCraft Schematic file (`.mts`) or through raw data supplied through Lua,
...
features, perhaps `freecraft.set_node()` is better suited for the job.
...
`freecraft.set_node()` loop may be more optimal. Always profile code using both
methods of map manipulation to determine which is most appropriate for your
usage.

A recent simple test of setting cubic areas showed that `freecraft.set_node()`
...
`VoxelManip([p1, p2])`, or `freecraft.get_voxel_manip([p1, p2])`.
...
`freecraft.get_content_id()` to look up the Content ID for the specified node
name, and `freecraft.get_name_from_content_id()` to look up the node name string
...
* `freecraft.CONTENT_UNKNOWN`: ID for "unknown" nodes
* `freecraft.CONTENT_AIR`:     ID for "air" nodes
* `freecraft.CONTENT_IGNORE`:  ID for "ignore" nodes
...
* The Mapgen VoxelManip object is retrieved using:
  `freecraft.get_mapgen_object("voxelmanip")`
...
* This VoxelManip object already has the region of map just generated loaded
  ...
  parameters of `on_generated()`. Refer to `freecraft.get_mapgen_object` docs.
...
* The `on_generated()` callbacks of some mods may place individual nodes in the
  ...
  `freecraft.add_node()`, `freecraft.set_node()` or `freecraft.swap_node()`
...
The functions `freecraft.generate_ores()` and `freecraft.generate_decorations()`
will generate all registered decorations and ores throughout the full area
inside of the specified VoxelManip object.

`freecraft.place_schematic_on_vmanip()` is otherwise identical to
`freecraft.place_schematic()`, except instead of placing the specified schematic
...
* If you attempt to use a VoxelManip to read a region of the map that has
  ...
  VoxelManip has a similar effect as calling `freecraft.load_area` on that
  region.
...
* If a region of the map has either not yet been generated or is outside the
  ...
  can use `freecraft.emerge_area` to make sure that the area you want to
...
* `write_to_map([light])`: Writes the data loaded from the `VoxelManip` back to
  the map.
    ...
    * if `light` is true, then lighting is automatically recalculated.
      ...
      all modified blocks with `freecraft.fix_light()` as soon as possible.
...
* `was_modified()`: Returns `true` or `false` if the data in the voxel
  ...
  `freecraft.set_data()` on the loaded area elsewhere.
...
The coordinates are *inclusive*, like most other things in FreeCraft.
...
`freecraft.get_mapgen_object()` function. If the requested Mapgen object is
...
call `freecraft.set_gen_notify()` first.
...
numeric unique decoration ID as returned by `freecraft.get_decoration_id()`.
...
* `on_deactivate(self, removal)`
	...
	* Note that this won't be called if the object hasn't been activated in the first place.
	  In particular, `freecraft.clear_objects({mode = "full"})` won't call this,
	  whereas `freecraft.clear_objects({mode = "quick"})` might call this.
...
freecraft.spawn_tree(pos,apple_tree)
```
...
A mod can register a custom privilege using `freecraft.register_privilege` function
...
A mod can call `freecraft.check_player_privs` to test whether a player has privileges
to perform an operation.
Also, when registering a chat command with `freecraft.register_chatcommand` a mod can
...
A mod can update player privileges using `freecraft.set_player_privs` function.
Players holding the `privs` privilege can see and manage privileges for all
players on the server.

A mod can subscribe to changes in player privileges using `freecraft.register_on_priv_grant`
and `freecraft.register_on_priv_revoke` functions.

Built-in privileges
-------------------

FreeCraft includes a set of built-in privileges that control capabilities
provided by the FreeCraft engine and can be used by mods:
...
FreeCraft includes the following settings to control behavior of privileges:
...
'freecraft' namespace reference
==============================

Utilities
---------

* `freecraft.get_current_modname()`: returns the currently loading mod's name,
  when loading a mod.
* `freecraft.get_modpath(modname)`: returns the directory path for a mod,
  e.g. `"/home/user/.freecraft/usermods/modname"`.
...
* `freecraft.get_modnames()`: returns a list of enabled mods, sorted alphabetically.
    * Does not include disabled mods, even if they are installed.
* `freecraft.get_game_info()`: returns a table containing information about the
...
* `freecraft.get_worldpath()`: returns e.g. `"/home/user/.freecraft/world"`
    * Useful for storing custom data
* `freecraft.is_singleplayer()`
* `freecraft.features`: Table containing API feature flags
  ...
  ```lua
  {
      ...
      -- Whether freecraft.find_path is functional (5.2.0)
...
* `freecraft.has_feature(arg)`: returns `boolean, missing_features`
    * `arg`: string or table in format `{foo=true, bar=true}`
    * `missing_features`: `{foo=true, bar=true}`
* `freecraft.get_player_information(player_name)`: Table containing information
...
* `freecraft.get_player_window_information(player_name)`:
  ...
  -- real_*_scaling is just OS DPI / 96 but with another level of user configuration.
  {
      ...
      -- Estimated maximum formspec size before FreeCraft will start shrinking the
...
* `freecraft.mkdir(path)`: returns success.
    * Creates a directory specified by `path`, creating parent directories
      if they don't exist.
* `freecraft.rmdir(path, recursive)`: returns success.
...
* `freecraft.get_version()`: returns a table containing components of the
   engine version.  Components:
    * `project`: Name of the project, eg, "FreeCraft"
  ...
  `if freecraft.check_for_falling then ... end`.
* `freecraft.sha1(data, [raw])`: returns the sha1 hash of data
    * `data`: string of data to hash
    * `raw`: return raw bytes instead of hex digits, default: false
* `freecraft.colorspec_to_colorstring(colorspec)`: Converts a ColorSpec to a
  ColorString. If the ColorSpec is invalid, returns `nil`.
    * `colorspec`: The ColorSpec to convert
* `freecraft.colorspec_to_bytes(colorspec)`: Converts a ColorSpec to a raw
  string of four bytes in an RGBA layout, returned as a string.
  * `colorspec`: The ColorSpec to convert
* `freecraft.encode_png(width, height, data, [compression])`: Encode a PNG
...
* `freecraft.debug(...)`
    * Equivalent to `freecraft.log(table.concat({...}, "\t"))`
* `freecraft.log([level,] text)`
...
* `freecraft.register_node(name, node definition)`
* `freecraft.register_craftitem(name, item definition)`
* `freecraft.register_tool(name, item definition)`
* `freecraft.override_item(name, redefinition)`
    * Overrides fields of an item registered with register_node/tool/craftitem.
    * Note: Item must already be defined, (opt)depend on the mod defining it.
    * Example: `freecraft.override_item("default:mese",
      {light_source=freecraft.LIGHT_MAX})`
* `freecraft.unregister_item(name)`
    * Unregisters the item from the engine, and deletes the entry with key
      `name` from `freecraft.registered_items` and from the associated item table
      according to its nature: `freecraft.registered_nodes`, etc.
* `freecraft.register_entity(name, entity definition)`
* `freecraft.register_abm(abm definition)`
* `freecraft.register_lbm(lbm definition)`
* `freecraft.register_alias(alias, original_name)`
    * Also use this to set the 'mapgen aliases' needed in a game for the core
      mapgens. See [Mapgen aliases] section above.
* `freecraft.register_alias_force(alias, original_name)`
* `freecraft.register_ore(ore definition)`
...
* `freecraft.register_biome(biome definition)`
    * Returns an integer object handle uniquely identifying the registered
      biome on success. To get the biome ID, use `freecraft.get_biome_id`.
* `freecraft.unregister_biome(name)`
    * Unregisters the biome from the engine, and deletes the entry with key
      `name` from `freecraft.registered_biomes`.
    * Warning: This alters the biome to biome ID correspondences, so any
      ...
      and re-registered.
* `freecraft.register_decoration(decoration definition)`
    * Returns an integer object handle uniquely identifying the registered
      decoration on success. To get the decoration ID, use
      `freecraft.get_decoration_id`.
    * The order of decoration registrations determines the order of decoration
      generation.
* `freecraft.register_schematic(schematic definition)`
    * Returns an integer object handle uniquely identifying the registered
...
* `freecraft.clear_registered_ores()`
    * Clears all ores currently registered.
* `freecraft.clear_registered_schematics()`
    * Clears all schematics currently registered.

### Gameplay

* `freecraft.register_craft(recipe)`
    * Check recipe table syntax for different types below.
* `freecraft.clear_craft(recipe)`
    * Will erase existing craft based either on output item or on input recipe.
    * Specify either output or input only. If you specify both, input will be
      ignored. For input use the same recipe table syntax as for
      `freecraft.register_craft(recipe)`. For output specify only the item,
      without a quantity.
    * Returns false if no erase candidate could be found, otherwise returns true.
    * **Warning**! The type field ("shaped", "cooking" or any other) will be
      ignored if the recipe contains output. Erasing is then done independently
      from the crafting method.
* `freecraft.register_chatcommand(cmd, chatcommand definition)`
* `freecraft.override_chatcommand(name, redefinition)`
    * Overrides fields of a chatcommand registered with `register_chatcommand`.
* `freecraft.unregister_chatcommand(name)`
    * Unregisters a chatcommands registered with `register_chatcommand`.
* `freecraft.register_privilege(name, definition)`
    ...
    * To allow players with `basic_privs` to grant, see the `basic_privs`
      freecraft.conf setting.
* `freecraft.register_authentication_handler(authentication handler definition)`
...
* `freecraft.register_globalstep(function(dtime))`
    * Called every server step, usually interval of 0.1s
* `freecraft.register_on_mods_loaded(function())`
    * Called after mods have finished loading and before the media is cached or the
      aliases handled.
* `freecraft.register_on_shutdown(function())`
    * Called before server shutdown
    * **Warning**: If the server terminates abnormally (i.e. crashes), the
      registered callbacks **will likely not be run**. Data should be saved at
      semi-frequent intervals as well as on server shutdown.
* `freecraft.register_on_placenode(function(pos, newnode, placer, oldnode, itemstack, pointed_thing))`
    * Called when a node has been placed
    * If return `true` no item is taken from `itemstack`
    * `placer` may be any valid ObjectRef or nil.
    * **Not recommended**; use `on_construct` or `after_place_node` in node
      definition whenever possible.
* `freecraft.register_on_dignode(function(pos, oldnode, digger))`
    * Called when a node has been dug.
    * **Not recommended**; Use `on_destruct` or `after_dig_node` in node
      definition whenever possible.
* `freecraft.register_on_punchnode(function(pos, node, puncher, pointed_thing))`
    * Called when a node is punched
* `freecraft.register_on_generated(function(minp, maxp, blockseed))`
    * Called after generating a piece of world. Modifying nodes inside the area
      is a bit faster than usual.
* `freecraft.register_on_newplayer(function(ObjectRef))`
    * Called when a new player enters the world for the first time
* `freecraft.register_on_punchplayer(function(player, hitter, time_from_last_punch, tool_capabilities, dir, damage))`
...
* `freecraft.register_on_rightclickplayer(function(player, clicker))`
...
* `freecraft.register_on_player_hpchange(function(player, hp_change, reason), modifier)`
...
* `freecraft.register_on_dieplayer(function(ObjectRef, reason))`
    * Called when a player dies
    * `reason`: a PlayerHPChangeReason table, see register_on_player_hpchange
* `freecraft.register_on_respawnplayer(function(ObjectRef))`
    * Called when player is to be respawned
    * Called _before_ repositioning of player occurs
    * return true in func to disable regular player placement
* `freecraft.register_on_prejoinplayer(function(name, ip))`
    ...
    * If it returns a string, the client is disconnected with that string as
      reason.
* `freecraft.register_on_joinplayer(function(ObjectRef, last_login))`
    * Called when a player joins the game
    * `last_login`: The timestamp of the previous login, or nil if player is new
* `freecraft.register_on_leaveplayer(function(ObjectRef, timed_out))`
    * Called when a player leaves the game
    * `timed_out`: True for timeout, false for other reasons.
* `freecraft.register_on_authplayer(function(name, ip, is_success))`
...
* `freecraft.register_on_auth_fail(function(name, ip))`
    * Deprecated: use `freecraft.register_on_authplayer(name, ip, is_success)` instead.
* `freecraft.register_on_cheat(function(ObjectRef, cheat))`
    ...
    * `cheat`: `{type=<cheat_type>}`, where `<cheat_type>` is one of:
        ...
        * `dug_unbreakable`
        * `dug_too_fast`
* `freecraft.register_on_chat_message(function(name, message))`
    * Called always when a player says something
    * Return `true` to mark the message as handled, which means that it will
      not be sent to other players.
* `freecraft.register_on_chatcommand(function(name, command, params))`
    * Called always when a chatcommand is triggered, before `freecraft.registered_chatcommands`
...
* `freecraft.register_on_player_receive_fields(function(player, formname, fields))`
    ...
    * Fields are sent for formspec elements which define a field. `fields`
      ...
      formspec element type:
        ...
        * `textlist`: See `freecraft.explode_textlist_event`
        * `table`: See `freecraft.explode_table_event`
        * `scrollbar`: See `freecraft.explode_scrollbar_event`
...
* `freecraft.register_on_craft(function(itemstack, player, old_craft_grid, craft_inv))`
    ...
    * Return either an `ItemStack`, to replace the output, or `nil`, to not
      modify it.
* `freecraft.register_craft_predict(function(itemstack, player, old_craft_grid, craft_inv))`
    * The same as before, except that it is called before the player crafts, to
      make craft prediction, and it should not change anything.
* `freecraft.register_allow_player_inventory_action(function(player, action, inventory, inventory_info))`
    * Determines how much of a stack may be taken, put or moved to a
      player inventory.
    * Function arguments: see `freecraft.register_on_player_inventory_action`
    * Return a numeric value to limit the amount of items to be taken, put or
      moved. A value of `-1` for `take` will make the source stack infinite.
* `freecraft.register_on_player_inventory_action(function(player, action, inventory, inventory_info))`
...
* `freecraft.register_on_protection_violation(function(pos, name))`
    * Called by `builtin` and mods when a player violates protection at a
      position (eg, digs a node or punches a protected entity).
    * The registered functions can be called using
      `freecraft.record_protection_violation`.
    * The provided function should check that the position is protected by the
      mod calling this function before it prints a message, if it does, to
      allow for multiple protection mods.
* `freecraft.register_on_item_eat(function(hp_change, replace_with_item, itemstack, user, pointed_thing))`
    * Called when an item is eaten, by `freecraft.item_eat`
    * Return `itemstack` to cancel the default item eat response (i.e.: hp increase).
* `freecraft.register_on_item_pickup(function(itemstack, picker, pointed_thing, time_from_last_punch,  ...))`
    * Called by `freecraft.item_pickup` before an item is picked up.
    * Function is added to `freecraft.registered_on_item_pickups`.
...
* `freecraft.register_on_priv_grant(function(name, granter, priv))`
    * Called when `granter` grants the priv `priv` to `name`.
    * Note that the callback will be called twice if it's done by a player,
      once with granter being the player name, and again with granter being nil.
* `freecraft.register_on_priv_revoke(function(name, revoker, priv))`
    * Called when `revoker` revokes the priv `priv` from `name`.
    * Note that the callback will be called twice if it's done by a player,
      once with revoker being the player name, and again with revoker being nil.
* `freecraft.register_can_bypass_userlimit(function(name, ip))`
    * Called when `name` user connects with `ip`.
    * Return `true` to by pass the player limit
* `freecraft.register_on_modchannel_message(function(channel_name, sender, message))`
    * Called when an incoming mod channel message is received
    * You should have joined some channels to receive events.
    * If message comes from a server mod, `sender` field is an empty string.
* `freecraft.register_on_liquid_transformed(function(pos_list, node_list))`
...
* `freecraft.register_on_mapblocks_changed(function(modified_blocks, modified_block_count))`
    ...
    * `modified_blocks` is the set of modified mapblock position hashes. These
      are in the same format as those produced by `freecraft.hash_node_position`,
      and can be converted to positions with `freecraft.get_position_from_hash`.
...
* `freecraft.settings`: Settings object containing all of the settings from the
  main config file (`freecraft.conf`).
* `freecraft.setting_get_pos(name)`: Loads a setting from the main settings and
  parses it as a position (in the format `(1,2,3)`). Returns a position or nil.

Authentication
--------------

* `freecraft.string_to_privs(str[, delim])`:
    * Converts string representation of privs into table form
    * `delim`: String separating the privs. Defaults to `","`.
    * Returns `{ priv1 = true, ... }`
* `freecraft.privs_to_string(privs[, delim])`:
    * Returns the string representation of `privs`
    * `delim`: String to delimit privs. Defaults to `","`.
* `freecraft.get_player_privs(name) -> {priv1=true,...}`
* `freecraft.check_player_privs(player_or_name, ...)`:
...
* `freecraft.check_password_entry(name, entry, password)`
...
* `freecraft.get_password_hash(name, raw_password)`
    * Convert a name-password pair to a password hash that FreeCraft can use.
    * The returned value alone is not a good basis for password checks based
      on comparing the password hash in the database with the password hash
      from the function, with an externally provided password, as the hash
      in the db might use the new SRP verifier format.
    * For this purpose, use `freecraft.check_password_entry` instead.
* `freecraft.get_player_ip(name)`: returns an IP address string for the player
  `name`.
    * The player needs to be online for this to be successful.

* `freecraft.get_auth_handler()`: Return the currently active auth handler
    * Must be called *after* load time, to ensure that any custom auth handler was
      already registered.
    * See the [Authentication handler definition]
    * Use this to e.g. get the authentication data for a player:
      `local auth_data = freecraft.get_auth_handler().get_auth(playername)`
* `freecraft.notify_authentication_modified(name)`
    * Must be called by the authentication handler for privilege changes.
    * `name`: string; if omitted, all auth data should be considered modified
* `freecraft.set_player_password(name, password_hash)`: Set password hash of
  player `name`.
* `freecraft.set_player_privs(name, {priv1=true,...})`: Set privileges of player
  `name`.
* `freecraft.auth_reload()`
    * See `reload()` in authentication handler definition

`freecraft.set_player_password`, `freecraft.set_player_privs`,
`freecraft.get_player_privs` and `freecraft.auth_reload` call the authentication
handler.

Chat
----

* `freecraft.chat_send_all(text)`
* `freecraft.chat_send_player(name, text)`
* `freecraft.format_chat_message(name, message)`
...
* `freecraft.set_node(pos, node)`
* `freecraft.add_node(pos, node)`: alias to `freecraft.set_node`
    * Set node at position `pos`
    * `node`: table `{name=string, param1=number, param2=number}`
    * If param1 or param2 is omitted, it's set to `0`.
    * e.g. `freecraft.set_node({x=0, y=10, z=0}, {name="default:wood"})`
* `freecraft.bulk_set_node({pos1, pos2, pos3, ...}, node)`
    * Set node on all positions set in the first argument.
    * e.g. `freecraft.bulk_set_node({{x=0, y=1, z=1}, {x=1, y=2, z=2}}, {name="default:stone"})`
    * For node specification or position syntax see `freecraft.set_node` call
...
* `freecraft.remove_node(pos)`
    * By default it does the same as `freecraft.set_node(pos, {name="air"})`
* `freecraft.get_node(pos)`
    * Returns the node at the given position as table in the format
      `{name="node_name", param1=0, param2=0}`,
      returns `{name="ignore", param1=0, param2=0}` for unloaded areas.
* `freecraft.get_node_or_nil(pos)`
    * Same as `get_node` but returns `nil` for unloaded areas.
* `freecraft.get_node_light(pos[, timeofday])`
...
* `freecraft.get_natural_light(pos[, timeofday])`
    ...
    * This function tests 203 nodes in the worst case, which happens very
      unlikely
* `freecraft.get_artificial_light(param1)`
...
* `freecraft.place_node(pos, node)`
    * Place node with the same effects that a player would cause
* `freecraft.dig_node(pos)`
    * Dig node with the same effects that a player would cause
    * Returns `true` if successful, `false` on failure (e.g. protected location)
* `freecraft.punch_node(pos)`
    * Punch node with the same effects that a player would cause
* `freecraft.spawn_falling_node(pos)`
    * Change node into falling node
    * Returns `true` and the ObjectRef of the spawned entity if successful, `false` on failure

* `freecraft.find_nodes_with_meta(pos1, pos2)`
    * Get a table of positions of nodes that have metadata within a region
      {pos1, pos2}.
* `freecraft.get_meta(pos)`
    * Get a `NodeMetaRef` at that position
* `freecraft.get_node_timer(pos)`
    * Get `NodeTimerRef`

* `freecraft.add_entity(pos, name, [staticdata])`: Spawn Lua-defined entity at
  position.
    * Returns `ObjectRef`, or `nil` if failed
* `freecraft.add_item(pos, item)`: Spawn item
    * Returns `ObjectRef`, or `nil` if failed
* `freecraft.get_player_by_name(name)`: Get an `ObjectRef` to a player
* `freecraft.get_objects_inside_radius(pos, radius)`: returns a list of
  ObjectRefs.
    * `radius`: using a Euclidean metric
* `freecraft.get_objects_in_area(pos1, pos2)`: returns a list of
  ObjectRefs.
     * `pos1` and `pos2` are the min and max positions of the area to search.
* `freecraft.set_timeofday(val)`
    * `val` is between `0` and `1`; `0` for midnight, `0.5` for midday
* `freecraft.get_timeofday()`
* `freecraft.get_gametime()`: returns the time, in seconds, since the world was
  created.
* `freecraft.get_day_count()`: returns number days elapsed since world was
  created.
    * accounts for time changes.
* `freecraft.find_node_near(pos, radius, nodenames, [search_center])`: returns
  pos or `nil`.
    * `radius`: using a maximum metric
    * `nodenames`: e.g. `{"ignore", "group:tree"}` or `"default:dirt"`
    * `search_center` is an optional boolean (default: `false`)
      If true `pos` is also checked for the nodes
* `freecraft.find_nodes_in_area(pos1, pos2, nodenames, [grouped])`
    * `pos1` and `pos2` are the min and max positions of the area to search.
    * `nodenames`: e.g. `{"ignore", "group:tree"}` or `"default:dirt"`
...
* `freecraft.find_nodes_in_area_under_air(pos1, pos2, nodenames)`: returns a
  list of positions.
    * `nodenames`: e.g. `{"ignore", "group:tree"}` or `"default:dirt"`
    * Return value: Table with all node positions with a node air above
    * Area volume is limited to 4,096,000 nodes
* `freecraft.get_perlin(noiseparams)`
    * Return world-specific perlin noise.
    * The actual seed used is the noiseparams seed plus the world seed.
* `freecraft.get_perlin(seeddiff, octaves, persistence, spread)`
    * Deprecated: use `freecraft.get_perlin(noiseparams)` instead.
    * Return world-specific perlin noise.
* `freecraft.get_voxel_manip([pos1, pos2])`
    * Return voxel manipulator object.
    * Loads the manipulator from the map if positions are passed.
* `freecraft.set_gen_notify(flags, {deco_ids})`
    ...
    * The second parameter is a list of IDs of decorations which notification
      is requested for.
* `freecraft.get_gen_notify()`
    * Returns a flagstring and a table with the `deco_id`s.
* `freecraft.get_decoration_id(decoration_name)`
    * Returns the decoration ID number for the provided decoration name string,
      or `nil` on failure.
* `freecraft.get_mapgen_object(objectname)`
    * Return requested mapgen object if available (see [Mapgen objects])
* `freecraft.get_heat(pos)`
    * Returns the heat at the position, or `nil` on failure.
* `freecraft.get_humidity(pos)`
    * Returns the humidity at the position, or `nil` on failure.
* `freecraft.get_biome_data(pos)`
    * Returns a table containing:
        * `biome` the biome id of the biome at that position
        * `heat` the heat at the position
        * `humidity` the humidity at the position
    * Or returns `nil` on failure.
* `freecraft.get_biome_id(biome_name)`
    * Returns the biome id, as used in the biomemap Mapgen object and returned
      by `freecraft.get_biome_data(pos)`, for a given biome_name string.
* `freecraft.get_biome_name(biome_id)`
    * Returns the biome name string for the provided biome id, or `nil` on
      failure.
    * If no biomes have been registered, such as in mgv6, returns `default`.
* `freecraft.get_mapgen_params()`
    * Deprecated: use `freecraft.get_mapgen_setting(name)` instead.
    * Returns a table containing:
        * `mgname`
        * `seed`
        * `chunksize`
        * `water_level`
        * `flags`
* `freecraft.set_mapgen_params(MapgenParams)`
    * Deprecated: use `freecraft.set_mapgen_setting(name, value, override)`
      instead.
    ...
    * `flags` is in the same format and has the same options as `mg_flags` in
      `freecraft.conf`.
* `freecraft.get_mapgen_edges([mapgen_limit[, chunksize]])`
...
* `freecraft.get_mapgen_setting(name)`
    * Gets the *active* mapgen setting (or nil if none exists) in string
      format with the following order of precedence:
        1) Settings loaded from map_meta.txt or overrides set during mod
           execution.
        2) Settings set by mods without a metafile override
        3) Settings explicitly set in the user config file, freecraft.conf
        4) Settings set as the user config default
* `freecraft.get_mapgen_setting_noiseparams(name)`
    * Same as above, but returns the value as a NoiseParams table if the
      setting `name` exists and is a valid NoiseParams.
* `freecraft.set_mapgen_setting(name, value, [override_meta])`
...
* `freecraft.set_mapgen_setting_noiseparams(name, value, [override_meta])`
    * Same as above, except value is a NoiseParams table.
* `freecraft.set_noiseparams(name, noiseparams, set_default)`
    ...
    * `set_default` is an optional boolean (default: `true`) that specifies
      ...
      active config.
* `freecraft.get_noiseparams(name)`
    * Returns a table of the noiseparams for name.
* `freecraft.generate_ores(vm, pos1, pos2)`
    * Generate all registered ores within the VoxelManip `vm` and in the area
      from `pos1` to `pos2`.
    * `pos1` and `pos2` are optional and default to mapchunk minp and maxp.
* `freecraft.generate_decorations(vm, pos1, pos2)`
    * Generate all registered decorations within the VoxelManip `vm` and in the
      area from `pos1` to `pos2`.
    * `pos1` and `pos2` are optional and default to mapchunk minp and maxp.
* `freecraft.clear_objects([options])`
    ...
    * The function signature of callback is:
      `function EmergeAreaCallback(blockpos, action, calls_remaining, param)`
        ...
        * `action` could be one of the following constant values:
            * `freecraft.EMERGE_CANCELLED`
            * `freecraft.EMERGE_ERRORED`
            * `freecraft.EMERGE_FROM_MEMORY`
            * `freecraft.EMERGE_FROM_DISK`
            * `freecraft.EMERGE_GENERATED`
...
* `freecraft.delete_area(pos1, pos2)`
    * delete all mapblocks in the area from pos1 to pos2, inclusive
* `freecraft.line_of_sight(pos1, pos2)`: returns `boolean, pos`
    * Checks if there is anything other than air between pos1 and pos2.
    * Returns false if something is blocking the sight.
    * Returns the position of the blocking node when `false`
    * `pos1`: First position
    * `pos2`: Second position
* `freecraft.raycast(pos1, pos2, objects, liquids)`: returns `Raycast`
...
* `freecraft.find_path(pos1,pos2,searchdistance,max_jump,max_drop,algorithm)`
    ...
    * `algorithm`: One of `"A*_noprefetch"` (default), `"A*"`, `"Dijkstra"`.
      ...
      on-the-fly
* `freecraft.spawn_tree (pos, {treedef})`
    * spawns L-system tree at given `pos` with definition in `treedef` table
* `freecraft.transforming_liquid_add(pos)`
    * add node to liquid flow update queue
* `freecraft.get_node_max_level(pos)`
    * get max available level for leveled node
* `freecraft.get_node_level(pos)`
    * get level of leveled node (water, snow)
* `freecraft.set_node_level(pos, level)`
    * set level of leveled node, default `level` equals `1`
    * if `totallevel > maxlevel`, returns rest (`total-max`).
* `freecraft.add_node_level(pos, level)`
    * increase level of leveled node by level, default `level` equals `1`
    * if `totallevel > maxlevel`, returns rest (`total-max`)
    * `level` must be between -127 and 127
* `freecraft.fix_light(pos1, pos2)`: returns `true`/`false`
...
* `freecraft.mod_channel_join(channel_name)`
    * Server joins channel `channel_name`, and creates it if necessary. You
      should listen for incoming messages with
      `freecraft.register_on_modchannel_message`

Inventory
---------

`freecraft.get_inventory(location)`: returns an `InvRef`

* `location` = e.g.
    * `{type="player", name="celeron55"}`
    * `{type="node", pos={x=, y=, z=}}`
    * `{type="detached", name="creative"}`
* `freecraft.create_detached_inventory(name, callbacks, [player_name])`: returns
  an `InvRef`.
...
* `freecraft.remove_detached_inventory(name)`
    * Returns a `boolean` indicating whether the removal succeeded.
* `freecraft.do_item_eat(hp_change, replace_with_item, itemstack, user, pointed_thing)`:
  returns leftover ItemStack or nil to indicate no inventory change
    * See `freecraft.item_eat` and `freecraft.register_on_item_eat`

Formspec
--------

* `freecraft.show_formspec(playername, formname, formspec)`
    * `playername`: name of player to show formspec
    * `formname`: name passed to `on_player_receive_fields` callbacks.
      It should follow the `"modname:<whatever>"` naming convention
    * `formspec`: formspec to display
* `freecraft.close_formspec(playername, formname)`
    ...
    * to close a formspec regardless of the formname, call
      `freecraft.close_formspec(playername, "")`.
      **USE THIS ONLY WHEN ABSOLUTELY NECESSARY!**
* `freecraft.formspec_escape(string)`: returns a string
    * escapes the characters "[", "]", "\", "," and ";", which cannot be used
      in formspecs.
* `freecraft.explode_table_event(string)`: returns a table
    * returns e.g. `{type="CHG", row=1, column=2}`
    * `type` is one of:
        * `"INV"`: no row selected
        * `"CHG"`: selected
        * `"DCL"`: double-click
* `freecraft.explode_textlist_event(string)`: returns a table
    * returns e.g. `{type="CHG", index=1}`
    * `type` is one of:
        * `"INV"`: no row selected
        * `"CHG"`: selected
        * `"DCL"`: double-click
* `freecraft.explode_scrollbar_event(string)`: returns a table
...
* `freecraft.inventorycube(img1, img2, img3)`
    * Returns a string for making an image of a cube (useful as an item image)
* `freecraft.get_pointed_thing_position(pointed_thing, above)`
    * Returns the position of a `pointed_thing` or `nil` if the `pointed_thing`
      does not refer to a node or entity.
    * If the optional `above` parameter is true and the `pointed_thing` refers
      to a node, then it will return the `above` position of the `pointed_thing`.
* `freecraft.dir_to_facedir(dir, is6d)`
...
* `freecraft.get_craft_recipe(output)`: returns input
...
* `freecraft.get_all_craft_recipes(query item)`: returns a table or `nil`
...
* `freecraft.handle_node_drops(pos, drops, digger)`
    * `drops`: list of itemstrings
    * Handles drops from nodes after digging: Default action is to put them
      into digger's inventory.
    * Can be overridden to get different functionality (e.g. dropping items on
      ground)
* `freecraft.itemstring_with_palette(item, palette_index)`: returns an item
  string.
...
* `freecraft.itemstring_with_color(item, colorstring)`: returns an item string
...
* `freecraft.rollback_get_node_actions(pos, range, seconds, limit)`:
  returns `{{actor, pos, time, oldnode, newnode}, ...}`
    * Find who has done something to a node, or near a node
    * `actor`: `"player:<name>"`, also `"liquid"`.
* `freecraft.rollback_revert_actions_by(actor, seconds)`: returns
...
* `freecraft.item_place_node(itemstack, placer, pointed_thing[, param2, prevent_after_place])`
...
* `freecraft.item_place_object(itemstack, placer, pointed_thing)`
    * Place item as-is
    * returns the leftover itemstack
    * **Note**: This function is deprecated and will never be called.
* `freecraft.item_place(itemstack, placer, pointed_thing[, param2])`
    * Wrapper that calls `freecraft.item_place_node` if appropriate
    * Calls `on_rightclick` of `pointed_thing.under` if defined instead
    * **Note**: is not called when wielded item overrides `on_place`
    * `param2` overrides facedir and wallmounted `param2`
    * returns `itemstack, position`
      * `position`: the location the node was placed to. `nil` if nothing was placed.
* `freecraft.item_pickup(itemstack, picker, pointed_thing, time_from_last_punch, ...)`
    * Runs callbacks registered by `freecraft.register_on_item_pickup` and adds
      the item to the picker's `"main"` inventory list.
    * Parameters are the same as in `on_pickup`.
    * Returns the leftover itemstack.
* `freecraft.item_drop(itemstack, dropper, pos)`
    * Drop the item
    * returns the leftover itemstack
* `freecraft.item_eat(hp_change[, replace_with_item])`
    * Returns `function(itemstack, user, pointed_thing)` as a
      function wrapper for `freecraft.do_item_eat`.
...
* `freecraft.node_punch(pos, node, puncher, pointed_thing)`
    * Calls functions registered by `freecraft.register_on_punchnode()`
* `freecraft.node_dig(pos, node, digger)`
    * Checks if node can be dug, puts item into inventory, removes node
    * Calls functions registered by `freecraft.registered_on_dignodes()`

Sounds
------

* `freecraft.sound_play(spec, parameters, [ephemeral])`: returns a handle
...
* `freecraft.sound_stop(handle)`
    * `handle` is a handle returned by `freecraft.sound_play`
* `freecraft.sound_fade(handle, step, gain)`
    * `handle` is a handle returned by `freecraft.sound_play`
...
* `freecraft.after(time, func, ...)`: returns job table to use as below.
...
`freecraft.get_node()` or `freecraft.get_player_by_name()` simply do not exist in it.
...
* `freecraft.handle_async(func, callback, ...)`:
    ...
    * Optional: Variable number of arguments that are passed to `func`
* `freecraft.register_async_dofile(path)`:
    * Register a path to a Lua file to be imported when an async environment
      is initialized. You can use this to preload code which you can then call
      later using `freecraft.handle_async()`.
...
* `freecraft.request_insecure_environment` (same restrictions apply)

Variables:
* `freecraft.settings`
* `freecraft.registered_items`, `registered_nodes`, `registered_tools`,
...
* `freecraft.request_shutdown([message],[reconnect],[delay])`: request for
  server shutdown. Will display `message` to clients.
    * `reconnect` == true displays a reconnect button
    * `delay` adds an optional delay (in seconds) before shutdown.
      Negative delay cancels the current active shutdown.
      Zero delay triggers an immediate shutdown.
* `freecraft.cancel_shutdown_requests()`: cancel current delayed shutdown
* `freecraft.get_server_status(name, joined)`
...
* `freecraft.get_server_uptime()`: returns the server uptime in seconds
* `freecraft.get_server_max_lag()`: returns the current maximum lag
  of the server in seconds or nil if server is not fully loaded yet
* `freecraft.remove_player(name)`: remove player from database (if they are not
  connected).
    * As auth data is not removed, freecraft.player_exists will continue to
      return true. Call the below method as well if you want to remove auth
      data too.
    * Returns a code (0: successful, 1: no such player, 2: player is connected)
* `freecraft.remove_player_auth(name)`: remove player authentication data
    * Returns boolean indicating success (false if player nonexistent)
* `freecraft.dynamic_add_media(options, callback)`
...
* `freecraft.get_ban_list()`: returns a list of all bans formatted as string
* `freecraft.get_ban_description(ip_or_name)`: returns list of bans matching
  IP address or name formatted as string
* `freecraft.ban_player(name)`: ban the IP of a currently connected player
    * Returns boolean indicating success
* `freecraft.unban_player_or_ip(ip_or_name)`: remove ban record matching
  IP address or name
* `freecraft.kick_player(name, [reason])`: disconnect a player with an optional
  reason.
    * Returns boolean indicating success (false if player nonexistent)
* `freecraft.disconnect_player(name, [reason])`: disconnect a player with an
...
* `freecraft.add_particle(particle definition)`
    * Deprecated: `freecraft.add_particle(pos, velocity, acceleration,
      expirationtime, size, collisiondetection, texture, playername)`

* `freecraft.add_particlespawner(particlespawner definition)`
    * Add a `ParticleSpawner`, an object that spawns an amount of particles
      over `time` seconds.
    * Returns an `id`, and -1 if adding didn't succeed
    * Deprecated: `freecraft.add_particlespawner(amount, time,
      minpos, maxpos,
      minvel, maxvel,
      minacc, maxacc,
...
* `freecraft.delete_particlespawner(id, player)`
    * Delete `ParticleSpawner` with `id` (return value from
      `freecraft.add_particlespawner`).
...
* `freecraft.create_schematic(p1, p2, probability_list, filename, slice_prob_list)`
        ...
        * `slice_prob_list` is an array of tables containing two fields, `ypos`
          and `prob`.
            ...
            * If slice probability list equals `nil`, no slice probabilities
              are applied.
    * Saves schematic in the FreeCraft Schematic format to filename.

* `freecraft.place_schematic(pos, schematic, rotation, replacements, force_placement, flags)`
...
* `freecraft.place_schematic_on_vmanip(vmanip, pos, schematic, rotation, replacement, force_placement, flags)`:
    * This function is analogous to freecraft.place_schematic, but places a
...
* `freecraft.serialize_schematic(schematic, format, options)`
...
* `freecraft.read_schematic(schematic, options)`
...
* `freecraft.request_http_api()`:
    ...
    * Only works at init time and must be called from the mod's main scope
      (not from a function).
    * Function only exists if freecraft server was built with cURL support.
...
* `freecraft.get_connected_players()`: returns list of `ObjectRefs`
* `freecraft.is_player(obj)`: boolean, whether `obj` is a player
* `freecraft.player_exists(name)`: boolean, whether player exists
  (regardless of online status)
* `freecraft.hud_replace_builtin(name, hud_definition)`
    * Replaces definition of a builtin hud element
    * `name`: `"breath"` or `"health"`
    * `hud_definition`: definition to replace builtin definition
* `freecraft.parse_relative_number(arg, relative_to)`: returns number or nil
    ...
    * Examples:
        * `freecraft.parse_relative_number("5", 10)` returns 5
        * `freecraft.parse_relative_number("~5", 10)` returns 15
        * `freecraft.parse_relative_number("~", 10)` returns 10
* `freecraft.send_join_message(player_name)`
    * This function can be overridden by mods to change the join message.
* `freecraft.send_leave_message(player_name, timed_out)`
    * This function can be overridden by mods to change the leave message.
* `freecraft.hash_node_position(pos)`: returns a 48-bit integer
    * `pos`: table {x=number, y=number, z=number},
    * Gives a unique hash number for a node position (16+16+16=48bit)
* `freecraft.get_position_from_hash(hash)`: returns a position
    * Inverse transform of `freecraft.hash_node_position`
* `freecraft.get_item_group(name, group)`: returns a rating
    * Get rating of a group of an item. (`0` means: not in group)
* `freecraft.get_node_group(name, group)`: returns a rating
    * Deprecated: An alias for the former.
* `freecraft.raillike_group(name)`: returns a rating
    * Returns rating of the connect_to_raillike group corresponding to name
    * If name is not yet the name of a connect_to_raillike group, a new group
      id is created, with that name.
* `freecraft.get_content_id(name)`: returns an integer
    * Gets the internal content ID of `name`
* `freecraft.get_name_from_content_id(content_id)`: returns a string
    * Gets the name of the content with that content ID
* `freecraft.parse_json(string[, nullvalue])`: returns something
    * Convert a string containing JSON data into the Lua equivalent
    * `nullvalue`: returned in place of the JSON null; defaults to `nil`
    * On success returns a table, a string, a number, a boolean or `nullvalue`
    * On failure outputs an error message and returns `nil`
    * Example: `parse_json("[10, {\"a\":false}]")`, returns `{10, {a = false}}`
* `freecraft.write_json(data[, styled])`: returns a string or `nil` and an error
  message.
    ...
    * Example: `write_json({10, {a = false}})`,
      returns `'[10, {"a": false}]'`
* `freecraft.serialize(table)`: returns a string
    * Convert a table containing tables, strings, numbers, booleans and `nil`s
      into string form readable by `freecraft.deserialize`
    * Example: `serialize({foo="bar"})`, returns `'return { ["foo"] = "bar" }'`
* `freecraft.deserialize(string[, safe])`: returns a table
    * Convert a string returned by `freecraft.serialize` into a table
...
* `freecraft.compress(data, method, ...)`: returns `compressed_data`
...
* `freecraft.decompress(compressed_data, method, ...)`: returns data
    * Decompress a string of data using the algorithm specified by `method`.
    * See documentation on `freecraft.compress()` for supported compression
      methods.
    * `...` indicates method-specific arguments. Currently, no methods use this
* `freecraft.rgba(red, green, blue[, alpha])`: returns a string
    * Each argument is an 8 Bit unsigned integer
    * Returns the ColorString from rgb or rgba values
    * Example: `freecraft.rgba(10, 20, 30, 40)`, returns `"#0A141E28"`
* `freecraft.encode_base64(string)`: returns string encoded in base64
    * Encodes a string in base64.
* `freecraft.decode_base64(string)`: returns string or nil on failure
    * Padding characters are only supported starting at version 5.4.0, where
      5.5.0 and newer perform proper checks.
    * Decodes a string encoded in base64.
* `freecraft.is_protected(pos, name)`: returns boolean
    ...
    * Example:
      ```lua
      local old_is_protected = freecraft.is_protected
      function freecraft.is_protected(pos, name)
      ...
      ```
* `freecraft.record_protection_violation(pos, name)`
    * This function calls functions registered with
      `freecraft.register_on_protection_violation`.
* `freecraft.is_creative_enabled(name)`: returns boolean
...
* `freecraft.is_area_protected(pos1, pos2, player_name, interval)`
    ...
    * Like `freecraft.is_protected`, this function may be extended or
      overwritten by mods to provide a faster implementation to check the
      cuboid for intersections.
* `freecraft.rotate_and_place(itemstack, placer, pointed_thing[, infinitestacks,
  orient_flags, prevent_after_place])`
    ...
    * `prevent_after_place` is directly passed to `freecraft.item_place_node`
    * Returns the new itemstack after placement
* `freecraft.rotate_node(itemstack, placer, pointed_thing)`
...
* `freecraft.calculate_knockback(player, hitter, time_from_last_punch,
...
* `freecraft.forceload_block(pos[, transient[, limit]])`
...
* `freecraft.forceload_free_block(pos[, transient])`
    * stops forceloading the position `pos`
    * If `transient` is `false` or absent, frees a persistent forceload.
      If `true`, frees a transient forceload.

* `freecraft.compare_block_status(pos, condition)`
...
* `freecraft.request_insecure_environment()`: returns an environment containing
...
* `freecraft.global_exists(name)`
    * Checks if a global variable has been set, without triggering a warning.

Global objects
--------------

* `freecraft.env`: `EnvRef` of the server environment and world.
    * Any function in the freecraft namespace can be called using the syntax
      `freecraft.env:somefunction(somearguments)`
      instead of `freecraft.somefunction(somearguments)`
...
to corresponding `freecraft.registered_*` tables.
...
* `AreaStore(type_name)`
    ...
    * `type_name`: optional, forces the internally used API.
        ...
        * When other values are specified, or SpatialIndex is not available,
          the custom FreeCraft functions are used.
...
* `get_location()`: returns a location compatible to
  `freecraft.get_inventory(location)`.
...
Can be obtained via `freecraft.get_meta(pos)`.
...
Can be gotten via `freecraft.get_node_timer(pos)`.
        ...
        * `sky_color`: A table used in `"regular"` type only, containing the
          following values (alpha is ignored):
            ...
            * `fog_tint_type`: string, changes which mode the directional fog
                ...
                `"default"` uses the classic FreeCraft sun and moon tinting.
...
It can be created via `PerlinNoise()` or `freecraft.get_perlin()`.
For `freecraft.get_perlin()`, the actual seed used is the noiseparams seed
...
`freecraft.get_perlin(noiseparams)`
`freecraft.get_perlin(seeddiff, octaves, persistence, spread)` (Deprecated).
...
`freecraft.get_perlin_map(noiseparams, size)`.
For `freecraft.get_perlin_map()`, the actual seed used is the noiseparams seed
plus the world seed, to create world-specific noise.
...
`freecraft.raycast(pos1, pos2, objects, liquids)` where:
...
An interface to read config files in the format of `freecraft.conf`.
...
* `has(key)`:
    ...
    * Note that for the main settings object (`freecraft.settings`), `get(key)`
...
Can be obtained via `freecraft.get_mod_storage()` during load time.
...
```lua
{
    ...
    -- For players this defaults to `freecraft.PLAYER_MAX_HP_DEFAULT`.

    breath_max = 0,
    -- For players only. Defaults to `freecraft.PLAYER_MAX_BREATH_DEFAULT`.
    ...
    --   `freecraft.itemstring_with_palette()`), the entity will inherit the color.
...
Used by `freecraft.register_entity`.
...
Used by `freecraft.register_abm`.
...
Used by `freecraft.register_lbm`.
...
Used by `freecraft.register_node`, `freecraft.register_craftitem`, and
`freecraft.register_tool`.
...
```lua
{
    ...
    -- To set the maximum (14), use the value 'freecraft.LIGHT_MAX'.
    -- A value outside the range 0 to freecraft.LIGHT_MAX causes undefined
    -- behavior.
    ...
    sound = {
        ...
        eat = <SimpleSoundSpec>,
        -- When item is eaten with `freecraft.do_item_eat`
...
Used by `freecraft.register_node`.
...
```lua
{
    ...
    -- `freecraft.set_node_level` and `freecraft.add_node_level`.
    ...
    -- Note: Within an on_construct callback, freecraft.set_node can cause an
    -- infinite loop if it invokes the same callback.
    --  Consider using freecraft.swap_node instead.
    -- default: nil
    ...
    -- freecraft.item_place_node / freecraft.place_node.
    -- If return true no item is taken from itemstack.
    -- `placer` may be any valid ObjectRef or nil.
    -- default: nil
    ...
    -- `freecraft.node_dig` / `freecraft.dig_node`.
    -- * `pos`: node position
    ...
    -- default: freecraft.node_punch
    -- Called when puncher (an ObjectRef) punches the node at pos.
    -- By default calls freecraft.register_on_punchnode callbacks.
    ...
    -- default: nil
    -- called by NodeTimers, see freecraft.get_node_timer and NodeTimerRef.
    ...
    -- See freecraft.register_on_player_receive_fields for more info.
    -- default: nil
...
Craft recipes are registered by `freecraft.register_craft` and use a
...
* `replacements`: (optional) Allows you to replace input items with some other items
    ...
    * Provided as a list of item pairs of the form `{ old_item, new_item }` where
      ...
      it will become
    * When the output is crafted, FreeCraft iterates through the list
...
the recipes can be retrieved later using `freecraft.get_craft_result` to
...
them via `freecraft.get_craft_result`.
...
Used by `freecraft.register_ore`.
...
Used by `freecraft.register_biome`.
...
See [Decoration types]. Used by `freecraft.register_decoration`.
...
```lua
{
    ...
    -- working directory of the specified FreeCraft schematic file.
...
Used by `freecraft.register_chatcommand`.
...
```lua
{
    ...
    privs = {},
    -- Required privileges to run. See `freecraft.check_player_privs()` for
...
Used by `freecraft.register_privilege`.
...
Used by `freecraft.create_detached_inventory`.
...
Used by `freecraft.add_particle`.
...
Used by `freecraft.add_particlespawner`.
...
```lua
{
    ...
    -- Optional, if specified replaces the default freecraft user agent with
    -- given string
...
Used by `freecraft.register_authentication_handler`.
...
When an error occurs that is not caught, FreeCraft calls the function
`freecraft.error_handler` with the error object as its first argument. The second
...
You can override `freecraft.error_handler`. You should call the previous handler
