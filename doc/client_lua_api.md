FreeCraft Lua Client Modding API Reference 5.8.0
================================================
...
Content and functionality can be added to FreeCraft 0.4.15-dev+ by using Lua
scripting in run-time loaded mods.

A mod is a self-contained bunch of scripts, textures and other related
things that is loaded by and interfaces with FreeCraft.
...
source code patches on GitHub (https://gitlab.com/KanuX/freecraft).
...
1) `$path_user/freecraft.conf` contains the setting `enable_client_modding = true`
...
* `RUN_IN_PLACE=0`: (Linux release)
    * `$path_share`:
        * Linux: `/usr/share/freecraft`
        * Windows: `<install directory>/freecraft-x.x.x`
    * `$path_user`:
        * Linux: `$HOME/.freecraft`
        * Windows: `C:/users/<user>/AppData/freecraft` (maybe)
...
* `freecraft-x.x.x/clientmods/` (User-installed mods)

On an installed version on Linux:

* `/usr/share/freecraft/clientmods/`
* `$HOME/.freecraft/clientmods/` (User-installed mods)
...
* See also `freecraft.explode_textlist_event` (main menu: `engine.explode_textlist_event`)
...
* See also `freecraft.explode_scrollbar_event` (main menu: `engine.explode_scrollbar_event`)
...
* See also `freecraft.explode_table_event` (main menu: `engine.explode_table_event`)
...
* `string:trim()`
    * e.g. `string.trim("\n \t\tfoo bar\t ") == "foo bar"`
* `freecraft.wrap_text(str, limit)`: returns a string
    * Adds new lines to the string to keep it within the specified character limit
    * limit: Maximal amount of characters in one line
* `freecraft.pos_to_string({x=X,y=Y,z=Z}, decimal_places))`: returns string `"(X,Y,Z)"`
    * Convert position to a printable string
      Optional: 'decimal_places' will round the x, y and z of the pos to the given decimal place.
* `freecraft.string_to_pos(string)`: returns a position
    * Same but in reverse. Returns `nil` if the string can't be parsed to a position.
* `freecraft.string_to_area("(X1, Y1, Z1) (X2, Y2, Z2)")`: returns two positions
...
* `freecraft.get_current_modname()`: returns the currently loading mod's name, when we are loading a mod
* `freecraft.get_modpath(modname)`: returns virtual path of given mod including
   the trailing separator. This is useful to load additional Lua files
   contained in your mod:
   e.g. `dofile(freecraft.get_modpath(freecraft.get_current_modname()) .. "stuff.lua")`
* `freecraft.get_language()`: returns two strings
...
* `freecraft.get_csm_restrictions()`: returns a table of `Flags` indicating the
...
* `freecraft.register_on_receiving_chat_message(function(message))`
    * Called always when a client receive a message
    * Return `true` to mark the message as handled, which means that it will not be shown to chat
* `freecraft.register_on_sending_chat_message(function(message))`
...
* `freecraft.register_chatcommand(cmd, chatcommand definition)`
    * Adds definition to freecraft.registered_chatcommands
* `freecraft.unregister_chatcommand(name)`
    * Unregisters a chatcommands registered with register_chatcommand.
* `freecraft.register_on_chatcommand(function(command, params))`
    * Called always when a chatcommand is triggered, before `freecraft.registered_chatcommands`
      is checked to see if the command exists, but after the input is parsed.
    * Return `true` to mark the command as handled, which means that the default
      handlers will be prevented.
* `freecraft.register_on_death(function())`
    * Called when the local player dies
* `freecraft.register_on_hp_modification(function(hp))`
    * Called when server modified player's HP
* `freecraft.register_on_damage_taken(function(hp))`
    * Called when the local player take damages
* `freecraft.register_on_formspec_input(function(formname, fields))`
    * Called when a button is pressed in the local player's inventory form
    * Newest functions are called first
    * If function returns `true`, remaining functions are not called
* `freecraft.register_on_dignode(function(pos, node))`
    * Called when the local player digs a node
    * Newest functions are called first
    * If any function returns true, the node isn't dug
* `freecraft.register_on_punchnode(function(pos, node))`
    * Called when the local player punches a node
    * Newest functions are called first
    * If any function returns true, the punch is ignored
* `freecraft.register_on_placenode(function(pointed_thing, node))`
    * Called when a node has been placed
* `freecraft.register_on_item_use(function(item, pointed_thing))`
...
* `freecraft.register_on_modchannel_signal(function(channel_name, signal))`
    ...
    * Possible values are:
      ...
      5: state_changed
* `freecraft.register_on_inventory_open(function(inventory))`
    * Called when the local player open inventory
    * Newest functions are called first
    * If any function returns true, inventory doesn't open
### Sounds
* `freecraft.sound_play(spec, parameters)`: returns a handle
    * `spec` is a `SimpleSoundSpec`
    * `parameters` is a sound parameter table
* `handle:stop()` or `freecraft.sound_stop(handle)`
    * `handle` is a handle returned by `freecraft.sound_play`
* `handle:fade(step, gain)` or `freecraft.sound_fade(handle, step, gain)`
    * `handle` is a handle returned by `freecraft.sound_play`
...
* `freecraft.find_node_near(pos, radius, nodenames, [search_center])`: returns pos or `nil`
...
### Player
* `freecraft.send_chat_message(message)`
    * Act as if `message` was typed by the player into the terminal.
* `freecraft.run_server_chatcommand(cmd, param)`
    * Alias for `freecraft.send_chat_message("/" .. cmd .. " " .. param)`
* `freecraft.clear_out_chat_queue()`
    * Clears the out chat queue
* `freecraft.localplayer`
    * Reference to the LocalPlayer object. See [`LocalPlayer`](#localplayer) class reference for methods.

### Privileges
* `freecraft.get_privilege_list()`
    * Returns a list of privileges the current player has in the format `{priv1=true,...}`
* `freecraft.string_to_privs(str)`: returns `{priv1=true,...}`
* `freecraft.privs_to_string(privs)`: returns `"priv1,priv2,..."`
    * Convert between two privilege representations

### Client Environment
* `freecraft.get_player_names()`
    * Returns list of player names on server (nil if CSM_RF_READ_PLAYERINFO is enabled by server)
* `freecraft.disconnect()`
    * Disconnect from the server and exit to main menu.
    * Returns `false` if the client is already disconnecting otherwise returns `true`.
* `freecraft.get_server_info()`
    * Returns [server info](#server-info).
* `freecraft.send_respawn()`
...
* `freecraft.mod_channel_join(channel_name)`
    * Client joins channel `channel_name`, and creates it, if necessary. You
      should listen from incoming messages with `freecraft.register_on_modchannel_message`
...
* `freecraft.delete_particlespawner(id)`
    * Delete `ParticleSpawner` with `id` (return value from `freecraft.add_particlespawner`)
...
* `freecraft.write_json(data[, styled])`: returns a string or `nil` and an error message
...
* `freecraft.deserialize(string)`: returns a table
    * Convert a string returned by `freecraft.deserialize` into a table
...
* `freecraft.gettext(string)` : returns string
    * look up the translation of a string in the gettext message catalog
* `fgettext_ne(string, ...)`
    * call freecraft.gettext(string), replace "$1"..."$9" with the given
      extra arguments and return the result
* `fgettext(string, ...)` : returns string
    * same as fgettext_ne(), but calls freecraft.formspec_escape before returning result
* `freecraft.pointed_thing_to_face_pos(placer, pointed_thing)`: returns a position
    * returns the exact position on the surface of a pointed node
* `freecraft.global_exists(name)`
    * Checks if a global variable has been set, without triggering a warning.

### UI
* `freecraft.ui.minimap`
...
* `freecraft.show_formspec(formname, formspec)` : returns true on success
	* Shows a formspec to the player
* `freecraft.display_chat_message(message)` returns true on success
...
* `freecraft.settings`: Settings object containing all of the settings from the
  main config file (`freecraft.conf`). Check lua_api.md for class reference.
...
-----------------
### Definitions
* `freecraft.get_node_def(nodename)`
	* Returns [node definition](#node-definition) table of `nodename`
* `freecraft.get_item_def(itemstring)`
...
```lua
{
	address = "freecraft.example.org", -- The domain name/IP address of a remote server or "" for a local server.
